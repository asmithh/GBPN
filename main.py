...
def run(dataset, split, model_name, dim_hidden, num_layers, num_hops, num_samples, dropout_p, device, learning_rate, num_epoches, initskip_BP, lossfunc_BP, weighted_BP, deg_scaling, learn_H, eval_C, verbose):
    ...
    if model_name == 'LP':
        ...
        model = GBPN(num_features, num_classes, dim_hidden=dim_hidden, num_layers=num_layers, activation=nn.ReLU(), dropout_p=dropout_p, lossfunc_BP=lossfunc_BP, deg_scaling=deg_scaling, learn_H=learn_H)
    else:
...
parser.add_argument('--lossfunc_BP', type=int, default=0)
